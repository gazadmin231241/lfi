...
`to-spec` и `to-tickets` для LFI-проектов: спеки и задачи попадают в локальный
трекер `.lfi`, а создание задач не выбирает конкретную модель, а назначает
абстрактный уровень выполнения.
...
и `[DONE]`. Спецификации никогда не исполняются. Задачи учитывают локальные
зависимости. Каждая исполняемая задача может содержать в документе
`execution_tier: light`, `standard` или `deep`. Отсутствующий уровень
выполняется как `standard` с предупреждением.
