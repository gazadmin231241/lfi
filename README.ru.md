...
нативные зависимости. Каждая исполняемая задача может содержать
`execution_tier: light`, `standard` или `deep`; в GitHub используется
соответствующая метка `lfi:tier:*`. Отсутствующий уровень выполняется как
`standard` с предупреждением.
...
Обычный интерактивный `lfi init` спрашивает источник задач, срок хранения логов
и нужно ли использовать рекомендуемое сопоставление Luna/Terra/Sol. Модели
...
этапов, GitHub mirror, default branch, команды подготовки и проверки доступны
в `.lfi/config.env`.
...
сохраняются, конфликтующие LFI type- и tier-labels исправляются. Частичный sync
...
уровень задачи модели Codex; пустая настройка уровня использует `CODEX_MODEL`.
`CODEX_REASONING_EFFORT` всегда передаётся ровно в заданном пользователем виде.
`MERGER_MODEL` независима и при отсутствии использует `STANDARD_MODEL`, затем
`CODEX_MODEL`. Если Codex отвергает явно настроенную модель worker-а, LFI явно
пишет об этом в лог, не подменяет её другой моделью, пропускает остальные задачи
с этой моделью в текущем запуске и продолжает другие уровни.
