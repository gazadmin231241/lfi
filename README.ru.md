...
проверка и итог. Сообщения агента остаются в live-потоке с префиксом, а команды,
token usage, подробности validation и `stderr` успешного процесса сохраняются
только в файлах.
...
Повторные попытки дописываются секциями со временем и номером итерации. Для
упавших worker-ов дополнительно сохраняется сжатый raw JSONL в
`.lfi/logs/failures/`.
...
Секции, диагностические файлы и старые timestamp-каталоги удаляются по
`LOG_RETENTION_DAYS` (по умолчанию три дня); значение `0` хранит их бессрочно.
